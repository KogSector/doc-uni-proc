//! doc-uni-proc - Vercel Serverless Entry Point
//!
//! This file provides a Vercel-compatible serverless handler for the unified processor.
//! For local development or long-running deployments, use the main.rs entry point instead.
...
// Global state for connection pooling across invocations
static mut PROCESSOR_STATE: Option<Arc<UnifiedProcessor>> = None;
static mut INITIALIZED: bool = false;

async fn initialize_processor() -> anyhow::Result<Arc<UnifiedProcessor>> {
    // Check if already initialized (unsafe for static mut, but acceptable for serverless singleton pattern)
    unsafe {
        if INITIALIZED {
            if let Some(ref processor) = PROCESSOR_STATE {
    ...
    // Note: Kafka consumer and keep-alive tasks are skipped in serverless mode
    // These should be handled by a separate long-running service

    // Store global state
    unsafe {
        PROCESSOR_STATE = Some(processor.clone());
        INITIALIZED = true;
    }
...
pub async fn handler(req: Request) -> Result<Response<String>, Error> {
    ...
    let auth_layer = unified_processor_lib::infra::middleware::AxumAuthLayer::new(
        processor.config().server.auth_middleware_url.clone(),
    );
    ...
    // Convert Vercel Request to Axum-compatible request
    let (parts, body) = req.into_parts();
    let uri = parts.uri.clone();
    let method = parts.method.clone();
    let headers = parts.headers.clone();

    // Create Axum request from parts
...
// Keep the original main for local development
#[tokio::main]
